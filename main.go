...
func main() {
	...
	CompareFile := flag.String("compare", "", "Cx1e2e JSON report file generated by cx1e2e to compare against")
...
func (c comparisonEntry) String() string {
	...
	if c.Result1 == c.Result2 {
		if c.Duration1 > 0 {
			ratio := 100.0*c.Duration2/c.Duration1 - 100.0
			var pct string
			if ratio > 0 {
				pct = fmt.Sprintf("+%.2f%%", ratio)
			} else {
				pct = fmt.Sprintf("%.2f%%", ratio)
			}
			return fmt.Sprintf("%v\t%d\t%v (%.2fs)\t%v (%.2fs)\t%v", parts[0], c.ID, c.Result1[:4], c.Duration1, c.Result2[:4], c.Duration2, pct)
		} else {
...
func CompareReports(reportFile1, reportFile2 string, logger *logrus.Logger) {
	...
	fmt.Fprintf(w, "Test\tTest ID\t%v\t%v\tDifference\n", reportFile1, reportFile2)
...
func (h *History) AddResult(re *regexp.Regexp, result ReportTestDetails, logger *logrus.Logger) {
	...
	if hist, ok := h.Pass[key]; ok {
		...
		if m > MaxPercentThreshold {
			logger.Warnf("Passing test %v duration %fs exceeded historical max by %d%%", key, result.Duration, m)
		}
		if a > AvgPercentThreshold {
			logger.Warnf("Passing test %v duration %fs exceeded historical average by %d%%", key, result.Duration, a)
		}
		h.Pass[key] = hist
	} else {
